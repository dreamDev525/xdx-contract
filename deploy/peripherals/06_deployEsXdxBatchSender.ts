...
const func: DeployFunction = async (hre) => {
  ...
  if (esXdxBatchSender.newlyDeployed) {
    console.log("make signals");
    let tx = await esXdxGov.signalSetHandler(esXdx.address, esXdxBatchSender.address, true);
    await tx.wait();
    tx = await xdxVesterGov.signalSetHandler(esXdx.address, esXdxBatchSender.address, true);
    await tx.wait();
    tx = await xlxVesterGov.signalSetHandler(esXdx.address, esXdxBatchSender.address, true);
    await tx.wait();
  }
};
